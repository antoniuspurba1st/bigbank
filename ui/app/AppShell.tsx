...
import { isSessionValid, clearSession, getSession } from "@/lib/session";
...
function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      // Setup interval to continually check expiration
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // We can also extract the user's email if needed
  ...
  return (
    <AuthGuard>
      ...
      <header className="main-header top-[33px]">
          ...
          <ul className="main-nav__list">
            {navLinks.map((link) => (
              <li key={link.href}>
                <Link 
                  href={link.href}
                  className={pathname === link.href ? "text-[color:var(--accent)] font-medium" : "flex items-center gap-1"}
                >
                  {link.label === "Demo" && (
        ...
        <div className="desktop-nav flex items-center gap-4">
          {session && (
            <span className="text-sm font-medium text-slate-500 overflow-hidden text-ellipsis max-w-[150px]">
              {session.email}
            </span>
          )}
      ...
      {/* Mobile Menu Panel */}
      {isMenuOpen && (
        <div className="mobile-nav__panel mt-[33px]">
            ...
            <ul className="space-y-4">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className={pathname === link.href ? "text-[color:var(--accent)] font-medium flex items-center gap-2" : "text-slate-600 flex items-center gap-2"}
